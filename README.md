...
3. **Keep it familiar, but don't get attached**: Keeping the same interface as the std character by character is not required, but I will try to keep it as similar as possible. The idea is to understand why some things in the std are the way they are, and see whether I reach the same conclusions or develop a different take.
