...
template <typename T1> using Rc = lstd::RcPtr<T1>;
...
lstd::Pair<int, int> pii(int i, int j) { return lstd::Pair<int, int>(i, j); }

lstd::Pair<int, int> pii(int i) { return lstd::Pair<int, int>(i); }

lstd::Pair<int, int> pii() { return lstd::Pair<int, int>(); }